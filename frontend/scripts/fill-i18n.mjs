...
const T = {
  ...
  'SheetView.legacyFormatTitle': { en: 'Visual preview not available for this format', it: 'Anteprima visiva non disponibile per questo formato', fr: 'Aperçu visuel indisponible pour ce format', de: 'Visuelle Vorschau für dieses Format nicht verfügbar', es: 'Vista previa visual no disponible para este formato', pt: 'Pré-visualização visual indisponível para este formato' },
  'SheetView.legacyFormatBody': { en: 'Legacy .xls / .xlsb / .ods files are not rendered in-app. The assistant can still answer questions about them — the text was extracted server-side. Use "Open in Word" to view the original.', it: 'I file legacy .xls / .xlsb / .ods non vengono visualizzati nell\'app. L\'assistente può comunque rispondere a domande su di essi — il testo è estratto lato server. Usa "Apri in Word" per vedere l\'originale.', fr: "Les fichiers anciens .xls / .xlsb / .ods ne sont pas rendus dans l'application. L'assistant peut quand même répondre aux questions sur leur contenu — le texte a été extrait côté serveur. Utilisez « Ouvrir dans Word » pour voir l'original.", de: 'Ältere Dateiformate .xls / .xlsb / .ods werden in der App nicht angezeigt. Der Assistent kann trotzdem Fragen dazu beantworten — der Text wurde serverseitig extrahiert. Verwenden Sie „In Word öffnen", um das Original anzuzeigen.', es: 'Los archivos antiguos .xls / .xlsb / .ods no se renderizan en la app. El asistente puede responder preguntas sobre ellos — el texto se extrajo del lado del servidor. Usa "Abrir en Word" para ver el original.', pt: 'Os ficheiros antigos .xls / .xlsb / .ods não são renderizados na aplicação. O assistente pode mesmo assim responder a perguntas sobre eles — o texto foi extraído do lado do servidor. Use "Abrir no Word" para ver o original.' },
