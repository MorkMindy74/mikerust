...
| `also_applicable_to` | *(built-in presets only, optional)* Array of **additional** domains the preset should also surface under, beyond the primary `domain`. A preset relevant to more than one vertical (e.g. fixed-asset analysis → both `fiscale` and `finance`) is registered **once** and listed in every applicable domain's picker — no duplicate JSON. Empty/omitted ⇒ shows strictly under `domain`. Each entry is validated against the canonical domain set at load (non-canonical or redundant-primary entries are dropped with a warning). Mirrors the DOCX-template field of the same name. User-created workflows (DB rows) are single-domain; this is a preset-only mechanism. |
